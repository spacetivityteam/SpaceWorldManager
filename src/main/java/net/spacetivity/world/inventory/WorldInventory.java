...
public class WorldInventory implements InventoryProvider {
    ...
    public static SmartInventory getInventory(Player player) {
        return SmartInventory.builder()
                .provider(new WorldInventory(player))
                .size(6, 9)
                .title(InventoryUtils.title("Worlds"))
                .build();
    }
    ...
    public void init(Player player, InventoryContents contents) {
        ...
        contents.set(0, 4, ClickableItem.empty(ItemBuilder.builder(Material.CHORUS_FLOWER, "§b§lWorlds")
                .setLores(List.of("§7Loaded: §f" + worldUtils.getAllWorldFiles().stream().filter(worldUtils::isWorldLoaded).toList().size(),
                        "§7Unloaded: §f" + worldUtils.getAllWorldFiles().stream().filter(file -> !worldUtils.isWorldLoaded(file)).toList().size(),
                        "§7Locked: §f" + worldUtils.getAllWorldFiles().stream().filter(worldUtils::isWorldLocked).toList().size()))
                .build()));
        ...
        loadAllWorlds(clickableItems -> {
            if (clickableItems.isEmpty()) {
                contents.set(SlotPos.of(2, 4), ClickableItem.empty(new ItemBuilder(Material.BARRIER)
                        .setDisplayName("§cNo worlds found!")
                        .build()));
            } else {
    ...
    public void getDisplayItem(World world, Consumer<ItemBuilder> result) {
        ItemBuilder itemBuilder = ItemBuilder.builder(Material.WRITABLE_BOOK, "§b§l" + world.getName())
                .setLores(List.of("§8Click for more options"))
